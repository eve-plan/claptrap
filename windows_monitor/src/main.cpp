...
void mqtt_callback(char *topic, byte *payload, unsigned int length) {
    ...
    if (strstr(topicStr.c_str(), topic_cpu_usage) != nullptr) {
        ...
        int newVoltage = percentageToVoltage(usage);
        Serial.printf("old_cpu_voltage: %d newVoltage: %d setup gpio: %d\n",
                      old_cpu_voltage, newVoltage,gpio_cpu_usage);
        if (newVoltage > old_cpu_voltage) {
            for (int i = old_cpu_voltage; i < newVoltage; ++i) {
                analogWrite(gpio_cpu_usage, i);
//                Serial.println(i);
                delay(10);
            }
        } else if (newVoltage < old_cpu_voltage) {
            for (int i = old_cpu_voltage; i > newVoltage; --i) {
                analogWrite(gpio_cpu_usage, i);
//                Serial.println(i);
                delay(10);
